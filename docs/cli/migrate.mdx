...
Migrate cldctl state between format versions.
...
In earlier versions of cldctl, environment state was stored as a peer to datacenters:
...
cldctl migrate state --backend s3 --backend-config bucket=my-cldctl-state

# Migrate with custom local path
cldctl migrate state --backend local --backend-config path=/opt/cldctl/state
```
