...
When you deploy a component, arcctl automatically deploys any missing dependencies into the target environment. This recursive resolution means a single `arcctl deploy` or `arcctl up` command brings up an entire application stack -- including all transitive dependencies.

- **Already deployed?** Dependencies that are already present in the environment are skipped.
- **Variables required?** In interactive mode, arcctl prompts for any required variables. In CI or `--auto-approve` mode, missing variables cause a clear error.
...
4. **Destroy protection**: You cannot destroy a component if other components in the environment depend on it. arcctl will list the dependents and block the operation. Use `--force` to override this check.
