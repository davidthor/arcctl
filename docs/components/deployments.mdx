...
deployments:
  api:
    command: ["npm", "run", "dev"]
    workingDirectory: ./backend  # Optional, defaults to cloud.component.yml directory
```
...
```yaml
# cloud.component.prod.yml (production - runs on VMs)
