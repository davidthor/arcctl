...
When observability is enabled and the datacenter's observability hook provides `query_type` and `query_endpoint` outputs, you can use `arcctl logs` to view and stream workload logs:
...
- [`arcctl observability dashboard`](/cli/observability/dashboard) - Open the monitoring dashboard
