...
Components can opt into OpenTelemetry observability with the `observability` block:

```yaml
# Enable with auto-injection of OTEL_* environment variables
observability:
  inject: true
  attributes:
    team: backend

# Or use expressions for manual wiring
observability:
  attributes:
    team: backend
...
When observability is enabled and the datacenter provides an observability hook, you can use `arcctl logs` to view and stream workload logs and `arcctl observability dashboard` to open the monitoring UI. See the [Observability Hook](/datacenters/observability-hook) for datacenter-side configuration.
