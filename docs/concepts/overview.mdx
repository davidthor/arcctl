...
When you deploy a component, arcctl recursively resolves its dependency tree. Any dependency not already present in the target environment is pulled and deployed automatically. This means a single `arcctl deploy` or `arcctl up` command brings up an entire multi-service application -- including transitive dependencies -- without manual orchestration.
