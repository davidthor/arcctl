...
1. **Declaration**: Platform engineers declare components in the datacenter file with their source and variable values
2. **Registration**: When the datacenter is deployed (`arcctl deploy datacenter`), component declarations are stored in the datacenter state
...
### Application Component (architect.yml)

```yaml
# architect.yml for myorg/web-app
dependencies:
  myorg/stripe:
    component: myorg/stripe
  myorg/clerk:
