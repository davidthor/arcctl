...
Datacenters typically define multiple `deployment` hooks with different `when` conditions to handle each mode. These hooks are evaluated **top-to-bottom in source order**, and **only the first matching hook is executed** for each deployment (waterfall-style):
