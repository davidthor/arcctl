...
| Output | Type | Description |
|--------|------|-------------|
| `query_type` | `string` | Log query backend type (e.g., `loki`). Required for `arcctl logs`. |
| `query_endpoint` | `string` | Log query endpoint URL. Required for `arcctl logs`. |
| `dashboard_url` | `string` | Observability dashboard URL (e.g., Grafana). Used by `arcctl observability dashboard`. |
...
3. **Component** `attributes` (from the `observability` block in `architect.yml`)
...
- [`arcctl logs`](/cli/logs) - View and stream logs
- [`arcctl observability dashboard`](/cli/observability/dashboard) - Open the dashboard UI
