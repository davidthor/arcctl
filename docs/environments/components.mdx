...
When using a local path (starting with `./`, `../`, or `/`), arcctl invokes the datacenter's `dockerBuild` hook to build images.
...
Variables defined in the component's `architect.yml` are resolved with these values at deployment time.
