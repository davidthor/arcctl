...
2. **Apply the environment** - Use `arcctl env apply` to deploy all components
3. **Manage lifecycle** - Update, scale, and destroy as needed

```mermaid
flowchart LR
    A[environment.yml] --> B[arcctl env apply]
    B --> C[Datacenter]
    C --> D[Cloud Resources]
...
```bash
arcctl component deploy my-app -e staging --config ghcr.io/myorg/my-app:v1.0.0
```
