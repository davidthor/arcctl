...
By default, arcctl looks up the **UPPER_SNAKE_CASE** version of the variable name:
...
arcctl automatically loads `.env` files from your **current working directory** (where you run the command). Files are loaded in order, with later files overriding earlier values:
...
The `{name}` is the environment name (the first argument to `arcctl update environment`).
...
```bash
arcctl update environment questra environments/questra.yml -d my-datacenter
```
...
- name: Deploy
  ...
  run: |
    arcctl update environment questra environments/questra.yml \
