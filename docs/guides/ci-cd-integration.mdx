    ...
    steps:
      ...
      - name: Build and deploy to staging
        run: |
          ...
          # Deploy to staging
          arcctl component deploy my-app \
            -e staging \
            --config ${{ secrets.ECR_REGISTRY }}/my-app:${{ github.sha }} \
            --auto-approve
```
      ...
      - name: Build and deploy
        run: |
          ...
          arcctl component deploy my-app \
            ...
            --config ghcr.io/${{ github.repository }}:pr-${{ github.event.number }} \
            --auto-approve
...
deploy-staging:
  ...
  script:
    - go install github.com/architect-io/arcctl@latest
    - arcctl component deploy my-app -e staging --config $CI_REGISTRY_IMAGE:$CI_COMMIT_SHA --auto-approve
  only:
    - main
  environment:
    name: staging
...
deploy-production:
  ...
  script:
    - go install github.com/architect-io/arcctl@latest
    - arcctl component deploy my-app -e production --config $CI_REGISTRY_IMAGE:$CI_COMMIT_SHA --auto-approve
  only:
    - tags
  environment:
    name: production
      ...
      - run:
          ...
          command: |
            arcctl component deploy my-app -e staging \
              --config ghcr.io/$CIRCLE_PROJECT_USERNAME/$CIRCLE_PROJECT_REPONAME:$CIRCLE_SHA1 \
              --auto-approve
...
arcctl component deploy my-app -e staging \
  --config ghcr.io/myorg/my-app:$CI_COMMIT_SHA \
  --auto-approve

# Avoid: Deploy latest (non-deterministic)
arcctl component deploy my-app -e staging \
  --config ghcr.io/myorg/my-app:latest \
  --auto-approve
```
...
- name: Deploy
  ...
  run: |
    arcctl component deploy my-app -e staging \
      --config ghcr.io/myorg/my-app:v1.0.0 \
      --var api_key=$API_KEY
```
