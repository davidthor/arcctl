...
This guide covers the operational details of running arcctl in CI/CD pipelines: configuring state backends, managing secrets, and implementing reliable deployment strategies.

## State Backend Configuration

arcctl needs a shared state backend in CI so that multiple pipeline runs and team members share the same infrastructure state. Configure the backend via environment variables:
...
- name: Deploy
  ...
  run: |
    arcctl deploy component ghcr.io/myorg/my-app:v1.0.0 -e staging \
      --var api_key=$API_KEY \
      --auto-approve
```
...
arcctl deploy component ghcr.io/myorg/my-app:$CI_COMMIT_SHA -e staging \
  --auto-approve

# Avoid: Deploy latest (non-deterministic)
arcctl deploy component ghcr.io/myorg/my-app:latest -e staging \
  --auto-approve
```
...
arcctl uses state locking to prevent concurrent modifications to the same environment. If a pipeline job fails mid-deploy, the lock may remain held. To recover:
...
arcctl build component . -t ghcr.io/myorg/my-app:v1.5.0
arcctl push component ghcr.io/myorg/my-app:v1.5.0

# Step 2: Deploy to staging
arcctl deploy component ghcr.io/myorg/my-app:v1.5.0 -e staging --auto-approve

# Step 3: Deploy to production (same artifact, no rebuild)
arcctl deploy component ghcr.io/myorg/my-app:v1.5.0 -e production --auto-approve
```
...
```bash
arcctl deploy component ghcr.io/myorg/my-app:v1.5.0 -e staging \
