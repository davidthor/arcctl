---
title: "CircleCI"
description: "Automate arcctl builds and deployments with CircleCI"
---

# CircleCI

This guide provides a complete CircleCI configuration for building and deploying components with arcctl.
...
Use a Go executor since arcctl is installed via `go install`:
...
|----------|-------------|
| `ARCCTL_BACKEND` | State backend type (e.g., `s3`) |
| `ARCCTL_BACKEND_S3_BUCKET` | S3 bucket for state storage |
| `ARCCTL_BACKEND_S3_REGION` | AWS region for state bucket |
