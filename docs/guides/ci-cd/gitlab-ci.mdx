---
title: "GitLab CI"
description: "Automate arcctl builds and deployments with GitLab CI/CD"
---

# GitLab CI

This guide provides a complete GitLab CI/CD pipeline for building, deploying, and promoting components with arcctl.
...
build:
  ...
  script:
    ...
    - arcctl build component . -t $CI_REGISTRY_IMAGE:$CI_COMMIT_SHA
    - arcctl push component $CI_REGISTRY_IMAGE:$CI_COMMIT_SHA
  only:
    - main
    - tags
...
deploy-staging:
  ...
  script:
    - go install github.com/architect-io/arcctl@latest
    - arcctl deploy component $CI_REGISTRY_IMAGE:$CI_COMMIT_SHA -e staging --auto-approve
  only:
    - main
  environment:
    name: staging
...
deploy-production:
  ...
  script:
    - go install github.com/architect-io/arcctl@latest
    - arcctl deploy component $CI_REGISTRY_IMAGE:$CI_COMMIT_SHA -e production --auto-approve
  only:
    - tags
  environment:
    name: production
