...
description: "How arcctl recursively deploys component dependencies to power ephemeral environments"
---

# Automatic Dependency Deployment

When you deploy a component, arcctl automatically detects, pulls, and deploys any dependency components that are not already present in the target environment. This recursive resolution is one of the key capabilities that makes ephemeral environments practical -- a single `arcctl deploy` or `arcctl up` brings an entire application stack online, no matter how many services it depends on.

## How It Works

When arcctl encounters a component with `dependencies`, it:
...
```mermaid
flowchart TD
    A["arcctl deploy component ./my-app -e preview-42"] --> B[Load my-app]
...
```
$ arcctl deploy component ./my-app -e preview-42
  ...
  deploy:
    ...
    steps:
      ...
      - name: Build component
        run: arcctl build component . -t ghcr.io/myorg/app:pr-${{ github.event.number }}

      - name: Push component
        run: arcctl push component ghcr.io/myorg/app:pr-${{ github.event.number }}

      - name: Create environment
        run: arcctl create environment preview-${{ github.event.number }} -d aws-staging

      - name: Deploy (with all dependencies)
        run: |
          arcctl deploy component ghcr.io/myorg/app:pr-${{ github.event.number }} \
  ...
  cleanup:
    ...
    steps:
      - name: Destroy preview
        run: arcctl destroy environment preview-${{ github.event.number }} --auto-approve
```
...
The `arcctl up` command uses the same recursive resolution:
...
Dependencies often require configuration -- API keys, secrets, project IDs. arcctl handles this differently depending on context:

### Interactive Mode (Terminal)

When running in a terminal, arcctl prompts for any required variables that the dependency needs and that weren't provided via `--var`:
...
In CI pipelines or when `--auto-approve` is set, arcctl cannot prompt. If a dependency has required variables without defaults, the command fails with an actionable error:
...
```bash
arcctl deploy component ghcr.io/myorg/app:v1.0.0 -e preview-42 \
...
arcctl deploy component ghcr.io/myorg/stripe:v1 -e preview-42 \
  --var secret_key=$STRIPE_SK --auto-approve

# Now deploy the main app -- stripe is already in the environment, skipped
arcctl deploy component ghcr.io/myorg/app:v1.0.0 -e preview-42 --auto-approve
```
...
When components are deployed with their dependency chain, arcctl records the relationships in state. This enables **destroy protection** -- if you try to destroy a component that other components depend on, arcctl blocks it:

```
$ arcctl destroy component clerk -e preview-42
...
Dependencies can themselves have dependencies. arcctl resolves the full transitive closure:
...
arcctl never updates a dependency that is already deployed. If `clerk` is already in the environment from a previous deployment, it is skipped entirely -- even if a newer version is available in the OCI registry. This is intentional:
...
```bash
arcctl deploy component ghcr.io/myorg/clerk:v2.0.0 -e staging
```
