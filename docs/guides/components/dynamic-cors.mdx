...
```bash
# Deploy API
arcctl component deploy api -e staging \
  --config ghcr.io/myorg/api-with-cors:v1.0.0 \
  --var jwt_secret=$JWT_SECRET

# Deploy frontend (automatically links to API)
arcctl component deploy frontend -e staging \
  --config ghcr.io/myorg/frontend-app:v1.0.0
```
