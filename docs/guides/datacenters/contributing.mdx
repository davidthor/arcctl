...
description: "How to contribute to arcctl's official datacenter templates"
---

# Contributing to Official Templates

arcctl's official datacenter templates live in the [`official-templates/`](https://github.com/architect-io/arcctl/tree/main/official-templates) directory at the root of the repository. They are maintained by the arcctl team and community contributors, and are intended for direct re-use by any organization adopting arcctl.
...
5. **What's Included** -- A summary of which resource hooks are implemented (database, deployment, function, service, ingress, bucket, etc.)
...
Before writing code, [open an issue](https://github.com/architect-io/arcctl/issues/new) describing:
...
git clone https://github.com/<your-username>/arcctl.git
cd arcctl
...
arcctl build datacenter ./official-templates/<name> \
  -t ghcr.io/<your-org>/test-dc:dev

# Deploy it
arcctl deploy datacenter test-dc ./official-templates/<name> \
  --var <required_var>=<value>

# Create an environment
arcctl create environment test-env -d test-dc

# Deploy a component to verify hooks work
arcctl deploy component ./examples/components/nextjs-app -e test-env

# Verify resources are healthy
arcctl list component -e test-env
arcctl logs -e test-env

# Clean up
arcctl destroy environment test-env --auto-approve
arcctl destroy datacenter test-dc --auto-approve
```
...
If you have questions about contributing, open a [GitHub Discussion](https://github.com/architect-io/arcctl/discussions) or reach out on the issue tracker.
