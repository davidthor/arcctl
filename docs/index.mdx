...
```mermaid
graph LR
    A[Component<br/>cloud.component.yml] --> C[cldctl]
    B[Datacenter<br/>datacenter.hcl] --> C
    C --> D[Environment]
    D --> E[Cloud Resources]
```

1. **Developers** write `cloud.component.yml` files describing their application's needs (databases, deployments, routes)
