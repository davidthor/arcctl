...
This component uses cldctl's `encryptionKeys` feature to automatically generate and manage cryptographic keys:
...
This component uses cldctl's `smtp` feature for email sending capabilities. The datacenter provisions SMTP credentials automatically, which may be fulfilled by various providers (AWS SES, SendGrid, Mailgun, etc.).
