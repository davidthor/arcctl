...
const (
	...
	EnvStateBackend = "ARCCTL_STATE_BACKEND"

	// EnvStatePrefix is the prefix for backend-specific config environment variables.
	// For example, ARCCTL_STATE_PATH sets the "path" config for the local backend,
	// ARCCTL_STATE_BUCKET sets the "bucket" config for S3/GCS backends.
	EnvStatePrefix = "ARCCTL_STATE_"
)
...
//  2. Environment variables (ARCCTL_STATE_BACKEND, ARCCTL_STATE_*)
//  3. Hardcoded defaults (local backend with ~/.arcctl/state)
func createStateManagerWithConfig(backendType string, backendConfig []string) (state.Manager, error) {
	...
	// Check for backend-specific env vars (ARCCTL_STATE_PATH, ARCCTL_STATE_BUCKET, etc.)
	for _, env := range os.Environ() {
		if strings.HasPrefix(env, EnvStatePrefix) && !strings.HasPrefix(env, EnvStateBackend) {
			parts := strings.SplitN(env, "=", 2)
			if len(parts) == 2 {
				// Convert ARCCTL_STATE_PATH to "path", ARCCTL_STATE_BUCKET to "bucket", etc.
