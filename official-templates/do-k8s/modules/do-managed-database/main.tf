...
locals {
  ...
  connection_url = local.engine == "redis" ? (
    ...
    "${var.type}://${digitalocean_database_cluster.db.user}:${digitalocean_database_cluster.db.password}@${digitalocean_database_cluster.db.host}:${digitalocean_database_cluster.db.port}/${digitalocean_database_cluster.db.database}"
  )
}
