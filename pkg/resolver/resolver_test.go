...
`
	...
	t.Run("directory without component file", func(t *testing.T) {
		...
		if err == nil {
			t.Error("Expected error for directory without architect.yml")
		}
	})
...
version: v1
`
	_ = os.WriteFile(filepath.Join(tmpDir1, "architect.yml"), []byte(componentContent), 0644)
	_ = os.WriteFile(filepath.Join(tmpDir2, "architect.yml"), []byte(componentContent), 0644)
...
func TestResolvedComponent(t *testing.T) {
	rc := ResolvedComponent{
		...
		Path:      "/absolute/path/to/architect.yml",
		Version:   "v1.0.0",
